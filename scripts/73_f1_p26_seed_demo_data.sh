...
echo "== F1-P26: seed demo data para validacion visual de Emisiones =="
echo "== Rama actual =="
...
if [[ "$(git branch --show-current)" != "feature/f1-p26" ]]; then
  echo "ERROR: este script debe ejecutarse en feature/f1-p26"
  echo "Ejecuta primero: git switch develop && git pull origin develop && git switch -c feature/f1-p26"
  exit 1
fi

mkdir -p scripts

cat > scripts/seed_f1_p26_demo_data.py <<'PY'
...
echo "== Ejecutar seed dentro del contenedor backend =="
docker compose exec -T backend python manage.py shell < scripts/seed_f1_p26_demo_data.py
...
echo "== OK F1-P26 aplicado. Revisar visualmente en http://localhost:8001 =="
